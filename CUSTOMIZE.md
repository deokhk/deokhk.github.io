...
│   └── 📂 json/
    │   └── 📄 resume.json: CV in JSON format (https://jsonresume.org/. Currently not supported)
├── 📂 _bibliography/
...
YOu can use a yml file located in [\_data/cv.yml](_data/cv.yml) to automatically generate a cv file. This was the original way of creating the CV page content and since it is more human readable than a json file we decided to keep it as an option.
